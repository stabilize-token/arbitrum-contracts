...
// File: contracts/strategies/StabilizeStrategyGLPV6.sol

pragma solidity =0.6.6;
pragma experimental ABIEncoderV2;

// This is a strategy is an absoluste stake strategy, fsGLP passively earns WETH, MPs + esGMX, GMX
...
interface GMXRewardRouter{
    ...
    function extendedGmxTracker() external view returns (address); // Reward tracker for GMX rewards for fsGLP
    function glpVester() external view returns (address); // Reward tracker for GMX rewards for vested fsGLP
    function gmxVester() external view returns (address); // Reward tracker for GMX rewards for vested GMX
...
contract StabilizeStrategyGLPV6 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public gasPrice = 1000000000; // 1 Gwei, governance can change
    ...
    address constant GMX_REWARD_ROUTER = address(0x5E4766F932ce00aA4a1A82d3Da85adf15C5694A1);
    ...
    address constant GMX_ADDRESS = address(0xfc5A1A6EB076a2C7aD06eD22C90d7E710E35ad0a);
    address constant USDC_ADDRESS = address(0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8); // USDC.e
    ...
    uint24 constant UNISWAPV3_FEE_MIDDLE_PERCENT = 3000; // 0.3%
    ...
    */
    ...
    function checkStakeProfit() internal returns (uint256) {
        if(buyMoreActive == false){return 0;} // Stop this feature
        ...
        // Calculate weth equivalent from swapping GMX
        uint256 gmxEarned = IERC20(GMX_ADDRESS).balanceOf(address(this));
        address extraGMX = GMXRewardRouter(GMX_REWARD_ROUTER).extendedGmxTracker();
        gmxEarned = GMXRewardTracker(extraGMX).claimable(address(this)).add(gmxEarned);
        extraGMX = GMXRewardRouter(GMX_REWARD_ROUTER).gmxVester();
        gmxEarned = GMXRewardTracker(extraGMX).claimable(address(this)).add(gmxEarned);
        extraGMX = GMXRewardRouter(GMX_REWARD_ROUTER).glpVester();
        gmxEarned = GMXRewardTracker(extraGMX).claimable(address(this)).add(gmxEarned);
        if(gmxEarned > 0){
            // Convert to WETH earned
            wethEarned = wethEarned.add(calculateUniswapV3Return(GMX_ADDRESS, WETH_ADDRESS, gmxEarned, UNISWAPV3_FEE_MIDDLE_PERCENT));
        }
        return wethEarned;
    }
    ...
    function redeemStakeProfit() internal {
        ...
        // bool _shouldClaimGmx,bool _shouldStakeGmx,bool _shouldClaimEsGmx,bool _shouldStakeEsGmx,
        // bool _shouldStakeMultiplierPoints,bool _shouldClaimWeth,bool _shouldConvertWethToEth
        GMXRewardRouter(GMX_REWARD_ROUTER).handleRewards(true,false,true,true,true,true,false); // Will pull out GMX and WETH (if available) and convert to more GLP
        ...
        _bal = IERC20(GMX_ADDRESS).balanceOf(address(this));
        if(_bal > 0){
            // We need to convert GMX to WETH
            swapViaUniswapV3(GMX_ADDRESS, WETH_ADDRESS, _bal, UNISWAPV3_FEE_MIDDLE_PERCENT);
        }
        ...
        if(_bal > 0){
            ...
            if(targetToken != WETH_ADDRESS){
                // Sell WETH on Uniswap for target token
                _bal = IERC20(targetToken).balanceOf(address(this));
                swapViaUniswapV3(WETH_ADDRESS, targetToken, sellAmount, UNISWAPV3_FEE_LOW_PERCENT);
    ...
    function calculateUniswapV3Return(address inputAddress, address outputAddress, uint256 _amount, uint24 _fee) internal returns (uint256) {
    ...
    function governanceStartMoveEsGMX(address _target) external onlyGovernance {
        ...
        if(GMXRewardRouter(GMX_REWARD_ROUTER).inStrictTransferMode() == true){
            // Must approve sbfGMX before signaling transfer to new account
            address sbfAddress = GMXRewardRouter(GMX_REWARD_ROUTER).feeGmxTracker();
            uint256 _sbfBal = GMXRewardTracker(sbfAddress).stakedAmounts(address(this));
            IERC20(sbfAddress).safeApprove(_target, 0);
            IERC20(sbfAddress).safeApprove(_target, _sbfBal);
        }
        GMXRewardRouter(GMX_REWARD_ROUTER).signalTransfer(_target); // Used to move esGMX
    }
    
    function governanceFinishMoveEsGMX(address _desired_router, address _sender) external onlyGovernance {
        // Obtain tokens from another address
        GMXRewardRouter(_desired_router).acceptTransfer(_sender); // Used to receive esGMX
    }
