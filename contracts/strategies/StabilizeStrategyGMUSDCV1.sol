...
// File: contracts/strategies/StabilizeStrategyGMUSDCV1.sol
...
interface StabilizeStrategyProperty {
    function bankAddress() external view returns (address);
    function zsTokenAddress() external view returns (address);

    function percentDepositor() external view returns (uint256);

    function maxTVL() external view returns (uint256);
    function callbackGasLimit() external view returns (uint256);
    function maxTradeSlippage() external view returns (uint256);
    function usdcMinMovement() external view returns (uint256);
    function usdcMinReserve() external view returns (uint256);
    function usdMinInterest() external view returns (uint256);
    function usdMaxMovement() external view returns (uint256);
    function minMarketCheckInterval() external view returns (uint256);
    function marketRewardsInfo(uint256) external view returns (uint256); 

    function interactGMMarkets() external view returns (bool);
    function emergencyWithdrawMode() external view returns (bool);

    function gasPrice() external view returns (uint256);
    function gasStipend() external view returns (uint256);
    function getGasFee() external view returns (uint256);

    // Some read functions
    function untaxBalance(address stratAddress, bool useFresh) external view returns (uint256);
    function balance(address stratAddress) external view returns (uint256);
    function getGMTokenPrice(address stratAddress, uint256 gmTokenIndex) external view returns (uint256);
...
contract StabilizeStrategyGMUSDCV1 is Ownable {
    using SafeMath for uint256;
    ...
    function determineInitialMarketPrices() external {
        ...
        for(uint256 it = 0; it < MarketList.length; it++){
            MarketList[it].lastComparedPrice = StabilizeStrategyProperty(propAddress).getGMTokenPrice(address(this), it);
    ...
    function onlyZSToken() internal view {
        require(StabilizeStrategyProperty(propAddress).zsTokenAddress() == _msgSender(), "E1");
    }
    ...
    function checkEmergencyWithdraw() internal view {
        require(StabilizeStrategyProperty(propAddress).emergencyWithdrawMode() == false, "E28");
    }
    ...
    function balance() public view returns (uint256) {
        ...
        return StabilizeStrategyProperty(propAddress).balance(address(this));
    }
    ...
    function deposit(bool nonContract, uint256 usdcDeposited) public {
        ...
        checkPendingCallbacks();
        compoundInterest();
        require(lastActionBalance.add(usdcDeposited.mul(1e18).div(10**tokenList[0].decimals)) < StabilizeStrategyProperty(propAddress).maxTVL(), "E27"); // Too much value locked in this contract for more deposits
        ...
        if(usdcDeposited == 0){
            ...
            require(usdcDeposited > StabilizeStrategyProperty(propAddress).usdcMinMovement().mul(10 ** tokenList[0].decimals), "E5");
    ...
    function withdraw(address _depositor, uint256 _share, uint256 _total, bool nonContract) public returns (uint256) {
        ...
        if(_share < _total){
            ...
            require(withdrawAmountUsdc > StabilizeStrategyProperty(propAddress).usdcMinMovement().mul(10 ** tokenList[0].decimals).mul(1e18).div(10 ** tokenList[0].decimals), "E8"); // Too little being withdrawn
        }else{
        ...
        if(_depositor != StabilizeStrategyProperty(propAddress).zsTokenAddress()){
            // Removing too much at once
            require(withdrawAmountUsdc < StabilizeStrategyProperty(propAddress).usdMaxMovement().mul(MarketList.length), "E9");
        }
        ...
        if(finalize == true){
            ...
            if(ok == true){
                lastActionBalance = StabilizeStrategyProperty(propAddress).untaxBalance(address(this), true);
    ...
    function finalizeWithdraw(address _receiver, uint256 _usdcAmount, bool takeAll) internal returns (bool) {
        ...
        if(recognizedUSDCBalance >= _usdcAmount){
            ...
            if(recognizedUSDCBalance < _usdcAmount.mul(DIVISION_FACTOR.sub(StabilizeStrategyProperty(propAddress).maxTradeSlippage())).div(DIVISION_FACTOR)){
    ...
    function compoundInterest() internal {
        for(uint256 it = 0; it < MarketList.length; it++){
            MarketList[it].recentPrice = StabilizeStrategyProperty(propAddress).getGMTokenPrice(address(this), it); // Update the recent prices
        }
        uint256 currentBalance = StabilizeStrategyProperty(propAddress).untaxBalance(address(this), false);
        if(currentBalance > lastActionBalance){
            uint256 newInterest = currentBalance.sub(lastActionBalance).mul(DIVISION_FACTOR.sub(StabilizeStrategyProperty(propAddress).percentDepositor())).div(DIVISION_FACTOR);
    ...
    function payInterestThenNextStep(uint256 nextStepFlag, uint256 amountUsdcMovedForUser, address _executor, address _user) internal returns (bool) {
        ...
        uint256 extraUsdc = StabilizeStrategyProperty(propAddress).calculateExtraUsdc(address(this));
        uint256 usdcNeeded;
        if(currentInterestDebt > StabilizeStrategyProperty(propAddress).usdMinInterest()){
        ...
        if(nextStepFlag == 1){ // Deposit only
            ...
            extraUsdc = StabilizeStrategyProperty(propAddress).calculateExtraUsdc(address(this));
            if(extraUsdc > 0){
            ...
            requestGMWithdraw(StabilizeStrategyProperty(propAddress).usdMaxMovement().mul(10 ** tokenList[0].decimals).div(1e18), false);
            return true;
        }
    }
    ...
    function requestGMWithdraw(uint256 _usdcAmountRemaining, bool _doMultiple) internal {
        ...
        while(true){ // Infinite loop possible
            ...
            uint256 _usdcAmount = _usdcAmountRemaining;
            {
                ...
                uint256 maxUsdcWithdraw = StabilizeStrategyProperty(propAddress).usdMaxMovement().mul(10 ** tokenList[0].decimals).div(1e18);
            ...
            _withdraw.callbackGasLimit = StabilizeStrategyProperty(propAddress).callbackGasLimit();
            ...
            {
                uint256 gmNeeded = 0;
                uint256 gmPrice = StabilizeStrategyProperty(propAddress).getGMTokenPrice(address(this), targetMarket);
                ...
                gmNeeded = gmNeeded.mul(1e18).div(gmPrice).mul(StabilizeStrategyProperty(propAddress).maxTradeSlippage().add(DIVISION_FACTOR)).div(DIVISION_FACTOR);
                ...
                ( , _withdraw.minShortTokenAmount) = StabilizeStrategyProperty(propAddress).getGMTokenWithdrawlAmountWithSlippage(address(this), targetMarket, gmNeeded);
            }
            ...
            uint256 gasCost;
            {
                gasCost = StabilizeStrategyProperty(propAddress).calculateUniswapV3Input(address(tokenList[0].token), WETH_ADDRESS, _withdraw.executionFee, UNISWAPV3_FEE_LOW_PERCENT);
    ...
    function requestGMDeposit(uint256 _usdcAmountForDeposit) internal {
        if(StabilizeStrategyProperty(propAddress).interactGMMarkets() == false) { return; }
        if(StabilizeStrategyProperty(propAddress).checkMarketFull(address(this), bestGMMarket) == true) { return; } // Cannot go into a full market

        {
            uint256 maxUsdcDeposit = StabilizeStrategyProperty(propAddress).usdMaxMovement().mul(10 ** tokenList[0].decimals).div(1e18);
        ...
        _deposit.callbackGasLimit = StabilizeStrategyProperty(propAddress).callbackGasLimit();
        {
            address datastoreAddress = GMXRouter(GMX_EXCHANGE_ROUTER).dataStore();
        ...
        {
            gasCost = StabilizeStrategyProperty(propAddress).calculateUniswapV3Input(address(tokenList[0].token), WETH_ADDRESS, _deposit.executionFee, UNISWAPV3_FEE_LOW_PERCENT);
...

        {
            ...
            _deposit.minMarketTokens = _deposit.minMarketTokens.mul(1e18).div(StabilizeStrategyProperty(propAddress).getGMTokenPrice(address(this), bestGMMarket));
            _deposit.minMarketTokens = _deposit.minMarketTokens.mul(DIVISION_FACTOR.sub(StabilizeStrategyProperty(propAddress).maxTradeSlippage())).div(DIVISION_FACTOR);
    ...
    function determineBestAndWorstMarkets() internal returns (uint256, uint256) {
        ...
        for(uint256 it = 0; it < MarketList.length; it++){
            uint256 cPrice = StabilizeStrategyProperty(propAddress).getGMTokenPrice(address(this), it);
            uint256 rewardFactor = StabilizeStrategyProperty(propAddress).marketRewardsInfo(it).mul(DIVISION_FACTOR).div(IERC20(MarketList[it].marketAddress).totalSupply()); // Normalize the reward
            ...
            MarketList[it].recentPrice = MarketList[it].lastComparedPrice;
            if(StabilizeStrategyProperty(propAddress).checkMarketFull(address(this), it) == false){
    ...
    function swapViaUniswapV3(address inputAddress, address outputAddress, uint256 _amount, uint24 _fee) internal returns (uint256) {
        ...
        return StabilizeStrategyProperty(propAddress).swapViaUniswapV3(inputAddress, outputAddress, address(this), _amount, _fee); // Then receive the tokens back
    }
    ...
    function afterDepositExecution(bytes32 key, ICallbackReceiver.DepositProps calldata _deposit, ICallbackReceiver.EventLogData calldata eventData) external {
        ...
        if(numberOfPendingCallbacks == 0){
            lastActionBalance = StabilizeStrategyProperty(propAddress).untaxBalance(address(this), false); // Using recent prices vs current
        }
    }
    ...
    function afterWithdrawalExecution(bytes32 key, ICallbackReceiver.WithdrawalProps calldata _withdraw, ICallbackReceiver.EventLogData calldata eventData) external {
        ...
        if(postWithdrawalAction == 2){
            if(numberOfPendingCallbacks == 0){
                ...
                // Burn user tokens
                if(ok == true){
                    zsToken(StabilizeStrategyProperty(propAddress).zsTokenAddress()).finalizeRedeem(postWithdrawUser); // This will burn the user's tokens
                }else{
                    zsToken(StabilizeStrategyProperty(propAddress).zsTokenAddress()).cancelRedeem(postWithdrawUser); // Something happened, maybe too much slippage, cancel the burn
                }
                lastActionBalance = StabilizeStrategyProperty(propAddress).untaxBalance(address(this), false);
            }
        }else if(postWithdrawalAction == 3){
    ...
    function afterWithdrawalCancellation(bytes32 key, ICallbackReceiver.WithdrawalProps calldata _withdraw, ICallbackReceiver.EventLogData calldata eventData) external {
        ...
        if(numberOfPendingCallbacks == 0){
            if(postWithdrawalAction == 2){
                zsToken(StabilizeStrategyProperty(propAddress).zsTokenAddress()).cancelRedeem(postWithdrawUser); // Cancel the redeem if that was the initial action
            }
            lastActionBalance = StabilizeStrategyProperty(propAddress).untaxBalance(address(this), false);
        }
    }
    ...
    function doSTBZBuyback(uint256 _amount) internal {
        ...
        StabilizeStrategyProperty(propAddress).doSTBZBuyback(_amount);
    }
    ...
    function payExecutor(address _executor) internal {
        ...
        uint256 gasFee = StabilizeStrategyProperty(propAddress).getGasFee();

        // Calculate how much usdc we need to pay the executor
        uint256 usdcNeeded = StabilizeStrategyProperty(propAddress).calculateUniswapV3Input(address(tokenList[0].token), WETH_ADDRESS, gasFee, UNISWAPV3_FEE_LOW_PERCENT);
    ...
    function checkExecutorPayoutCode() internal returns (uint256) {
        ...
        if(postWithdrawalAction != 3){
            if(block.timestamp < lastMarketCheck.add(StabilizeStrategyProperty(propAddress).minMarketCheckInterval())) { return 0; }
            lastMarketCheck = block.timestamp;
            uint256 cBestMarket = bestGMMarket;
            (bestGMMarket, worstGMMarket) = determineBestAndWorstMarkets();
            if(bestGMMarket != cBestMarket && worstGMMarket != bestGMMarket){
                // There is a new best market, take from the worst and put into best
                return 1;
            }
            return 0;
        }else{
    ...
    function governanceEmergencyWithdrawToken(address _token, uint256 _amount) external {
        onlyGovernance();
        require(StabilizeStrategyProperty(propAddress).emergencyWithdrawMode() == true, "E24");
...
contract StabilizeStrategyGMUSDCPropertiesV1 is Ownable, GasUtils, MarketUtils {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    uint256 public usdMinInterest = 100e18; // Minimum amount of interest needed to send it to the user
    uint256 public usdMaxMovement = 100000e18; // The maximum amount (in USD with 18 decimals) that can be deposited or withdrawn from a GM market at a time
    ...
    uint256 public minMarketCheckInterval = 1 days; // The time in between market checks that can be performed
