...
// File: contracts/strategies/StabilizeStrategyWBTCTraderV1.sol
...
contract StabilizeStrategyWBTCTraderV1 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public averagePricePeriod = 4 hours;
    uint256 public minTradeVolatility = 500; // 0.5% price deviation from Chainlink price will lead to trade
    uint256 public priceOracleMethod = 2; // 0 = Use only average price, 1 = Use only chainlink, 2 = Use combination of chainlink and average
    ...
    // Depositor info
    uint256 public percentDepositor = 99900; // 1000 = 1%, depositors keep 99.9% from trades
    ...
    */
    ...
    function calculateTokenAndAmountToSell() internal view returns (uint256, uint256) {
        ...
        uint256 tradeType = 0; // 0 = Nothing, 1 = Sell volatile, 2 = sell stable
        {
            ...
            if(priceOracleMethod == 1){
                chainlinkPrice = getBTCUSDPrice();
            }else if(priceOracleMethod == 0){
                chainlinkPrice = averagePrice;
            }else if(priceOracleMethod == 2){
                chainlinkPrice = averagePrice.add(getBTCUSDPrice()).div(2);
            }
            ...
            if(spotPrice > chainlinkPrice){
                uint256 volatility = spotPrice.sub(chainlinkPrice).mul(DIVISION_FACTOR).div(chainlinkPrice);
                if(volatility > minTradeVolatility){
                    tradeType = 1;
                }
            }else if(spotPrice < chainlinkPrice){
                uint256 volatility = chainlinkPrice.sub(spotPrice).mul(DIVISION_FACTOR).div(chainlinkPrice);
                if(volatility > minTradeVolatility){
                    tradeType = 2;
                }
            }
        ...
        if(tradeType == 1){
            ...
            uint256 _bal = tokenList[0].token.balanceOf(address(this));
            if(wbtcInUSDTUnits(_bal).mul(1e18).div(10**tokenList[1].decimals) < minSellAmount){
                // If we have less than a certain amount of tokens in the contract, sell it all
                sellAmount = DIVISION_FACTOR;
            }
            ...
            if(_bal.mul(1e18).div(10**tokenList[1].decimals) < minSellAmount){
                // If we have less than a certain amount of tokens in the contract, sell it all
                sellAmount = DIVISION_FACTOR;
            }
            sellAmount = _bal.mul(sellAmount).div(DIVISION_FACTOR);
    ...
    function updateAveragePrice() public {
        // This will update the average price if necessary
        if(block.timestamp < lastTimePriceTaken.add(averagePricePeriod)) {return;} // Too soon to update the price
        lastTimePriceTaken = block.timestamp;

        // We will use a combination of the chainlink and equal averages to determine the current price
        uint256 price = getBTCUSDPrice();
        uint256 newPrice = averagePrice.add(price).div(2);

        // Now determine market direction
        uint256 deviation = 0;
        if(newPrice > averagePrice){
            deviation = newPrice.sub(averagePrice).mul(DIVISION_FACTOR).div(averagePrice);
        }else if(newPrice < averagePrice){
            deviation = averagePrice.sub(newPrice).mul(DIVISION_FACTOR).div(averagePrice);
        }
    ...
    function changeTradingConditions(uint256 _cooldown, 
                                    ...
                                    uint256 _oracleMethod) external onlyGovernance {
        ...
        averagePricePeriod = _pricePeriod;
        ...
        priceOracleMethod = _oracleMethod;
    }
    // --------------------
