...
// File: contracts/strategies/StabilizeStrategyWBTCTraderV4.sol
...
// Strat only sells when the price direction is sideways
...
contract StabilizeStrategyWBTCTraderV4 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public lastPricePeriod = 4 hours;
    uint256 public minTradeVolatility = 1000; // 1% price deviation from last price will lead to trade
    uint256 public priceDirectionThreshold = 2000; // 2% movement required to update price direction
    ...
    */
    ...
    function calculateTokenAndAmountToSell() internal view returns (uint256, uint256) {
        ...
        uint256 tradeType = 0; // 0 = Nothing, 1 = Sell volatile, 2 = sell stable
        {
            ...
            if(spotPrice > currentPrice){
                ...
                if(volatility > priceDirectionThreshold){
                    // Too much movement, market not in swing
                    tradeType = 0;
                }
        ...
        if(tradeType == 0 || priceDirection != 0){
    ...
    function _updateLastPrice(bool forced) internal {
        ...
        if(deviation > priceDirectionThreshold){
            if(lastPrice > oldPrice){
                priceDirection = 1;
            }else{
                priceDirection = -1;
    ...
    function changeTradingConditions(uint256 _cooldown, 
                                    ...
                                    uint256 _priceDirectionThres,
                                    uint256 _slippage) external onlyGovernance {
        // Changes a lot of trading parameters in one call
        require( _percentSell <= 100000 && _slippage < 100000 && _priceDirectionThres <= 100000,"Percent cannot be greater than 100%");
        ...
        priceDirectionThreshold = _priceDirectionThres;
    }
    // --------------------
