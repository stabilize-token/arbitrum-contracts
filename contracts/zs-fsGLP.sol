...
// Users can only deposit and withdraw fsGLP
// Due to GMX unique transfer logic, we must use a wrapped fsGLP router to transfer fsGLP around

interface StabilizeStrategy {
    function withdrawLocked() external view returns (bool); // The strategy may sometimes lock due to GMX internal cooldown features that prevent transfers after liquidity added
...
contract zsToken is ERC20("Stabilize Arbitrum Strategy fsGLP", "zs-fsGLP"), Ownable, ReentrancyGuard {
    ...
    address constant WRAPPED_FSGLP = address(0x01AF26b74409d10e15b102621EDd29c326ba1c55);
    ...
    function withdrawLocked() external view returns (bool) {
        if(currentStrategy != StabilizeStrategy(address(0))){
            return currentStrategy.withdrawLocked();
        }else{
            return false;
        }
    }
    ...
    function deposit(uint256 amount) public nonReentrant {
        ...
        IERC20(WRAPPED_FSGLP).safeTransferFrom(_msgSender(), address(this), amount); // Transfer token to this address
        ...
        pushGLPToStrategy(); // Push any strategy tokens here into the strategy
    ...
    function finishChangeStrategy() public onlyGovernance timelockConditionsMet(2) {
        ...
        if(currentStrategy != StabilizeStrategy(address(0)) && totalSupply() > 0){
            pushGLPToStrategy(); // It will push any strategy reward tokens here to the new strategy
            currentStrategy.enter(); // Puts all the tokens and accessory tokens into the new strategy 
        }
        _pendingStrategy = address(0);
    }
    
    function pushGLPToStrategy() internal {
        ...
        for(uint256 i = 0; i < tokenCount; i++){
            ...
            if(_balance > 0){
                IERC20(WRAPPED_FSGLP).safeTransfer(address(currentStrategy), _balance);
            }
        }
    }
    // --------------------
