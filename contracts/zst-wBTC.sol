...
// Users by default withdraw from main token before collateral token
// When withdrawing from collateral token, it is automatically converted to main token
...
contract zstToken is ERC20("Stabilize Strategy Trader wBTC", "zst-wBTC"), Ownable, ReentrancyGuard {
    ...
    // This token container doesn't have a fee but the underlying strategies may for wBTC
    // This wrapper takes into account the value of wBTC compared to the collateral
    // Using the Chainlink price oracle, we convert USDT in the strategy to wBTC equivalent to determine the balance
    // When withdrawing and depositing, this equivalent will be used to determine how much zs tokens to mint
    // And how much wbtc to withdraw
    ...
    function curveSwapUSDTForWBTC(uint256 amount, address receiver) internal {
        CurveTriExchange curve = CurveTriExchange(CURVE_TRIPOOL);
        IERC20(USDT_ADDRESS).safeApprove(address(curve), 0);
        IERC20(USDT_ADDRESS).safeApprove(address(curve), amount);
        uint256 _bal = IERC20(WBTC_ADDRESS).balanceOf(address(this));
        curve.exchange(0, 1, amount, 1); // Arbitrum doesn't have front-running so this is not an issue
        amount = IERC20(WBTC_ADDRESS).balanceOf(address(this)).sub(_bal); // Get the amount of wbtc returned
                ...
                if(_bal > 0){
                    if(i == 0){
                        ...
                        if(convert == false){
                            ...
                            curveSwapUSDTForWBTC(_bal, _receiver); // Will convert USDT to WBTC
                        }
                    }
                ...
                if(_normalizedBalance > 0){
                    ...
                    if(i == 0){
                        ...
                        if(convert == false){
                            ...
                            curveSwapUSDTForWBTC(tokenList[i].token.balanceOf(address(this)), _receiver); // Will convert USDT to WBTC
                        }
                    }              
                }
            }else{
                ...
                if(_withdrawAmount > 0){
                    ...
                    if(i == 0){
                        ...
                        if(convert == false){
                            ...
                            curveSwapUSDTForWBTC(_balance, _receiver); // Will convert USDT to WBTC
                        }
                    }
                }
